...
def parse_args():
    ...
    parser.add_argument("--partition", type=str, default="iid", choices=["iid", "dirichlet"],
                         help="Client data partitioning strategy")
...
def split_train_data_dirichlet(train_dataset, num_clients=NUM_CLIENTS, alpha=0.5, seed=SEED):
    """Dirichlet-partitioned non-IID split. Lower alpha = more skewed."""
    labels = np.array([
        (s['label'].item() if torch.is_tensor(s['label']) else s['label'])
    ...
    print(f"\n=== Client Data Distribution (Dirichlet alpha={alpha}, Seed={seed}) ===")
    client_datasets = []
    for i, idx in enumerate(client_indices):
        ...
        print(f"Client {i} → {len(subset)} samples | Label distribution: {dict(zip(unique.tolist(), counts.tolist()))}")
    print("=" * 60)
    return client_datasets
...
def main(train_csv: str, test_csv: str):
    ...
    if ARGS.partition == "dirichlet":
        client_datasets = split_train_data_dirichlet(
            train_dataset, NUM_CLIENTS, alpha=ARGS.alpha, seed=SEED
        )
    else:
