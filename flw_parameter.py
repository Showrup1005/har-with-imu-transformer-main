...
class mp:
    ...
    @staticmethod
    def encode(flat: np.ndarray, num_bits: int) -> dict:
        n = flat.size
...
class IMUClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, fit_config):
        ...
        for name, new_val in new_state.items():
            ...
            payload = mp.encode(flat, num_bits)
...
class Strategy(fl.server.strategy.FedAvg):
    def __init__(self, test_loader, use_compression=USE_COMPRESSION,
                 ...
                 update_mode=UPDATE_MODE, server_lr=SERVER_LR, **kwargs):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        print(f"Round {server_round}/{NUM_ROUNDS} [{self.update_mode}] - Accuracy: {acc:.4f} | num_bits={self._current_num_bits}")
        ...
        if acc > self.best_acc:
            self.best_acc = acc
            torch.save(self.global_model.state_dict(), f"best_model_seed{SEED}_{self.update_mode}.pth")

        if server_round == NUM_ROUNDS:
            print("\n========== FINAL EVALUATION ==========")
            self.evaluate_global(final=True)
            self.save_run_data(path=f"fl_run_history_{self.update_mode}_seed{SEED}.npz")
...
def main(train_csv: str, test_csv: str):
    ...
    print(f"Starting FL | seed={SEED} | mode={UPDATE_MODE} | {NUM_CLIENTS} Clients | {NUM_ROUNDS} Rounds\n")
