...
class FedCVLCStrategy(fl.server.strategy.FedAvg):
    def __init__(self, test_loader, use_compression=USE_COMPRESSION,
        ...
        # element" figure below -- Fed-CVLC has no single per-round
        # bit-width the way FGMP does (four bands, each with its own
        # bit-width, plus a scheduled top_k_ratio on top). This is a
        # DERIVED number (bytes actually sent -> bits/element), stored
        # under "num_bits" purely so the FGMP plotting script's bit-width
        # panel has something to plot. The real scheduled quantity is
        # kept separately as "top_k_ratio" below -- read num_bits here as
        # "effective bits/element", not "quantizer bits".
        ...
        # save_run_data(). Same core keys as the other scripts' history
        # dicts, plus Fed-CVLC's own scheduled top_k_ratio.
        self.history = {
            "round": [],
            "accuracy": [],
            "num_bits": [],
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        self.history["top_k_ratio"].append(self._current_top_k)
    ...
    def save_run_data(self, path="fl_run_history_fedcvlc.npz"):
        h = self.history
        ...
        save_kwargs.update({
            ...
            "top_k_ratio_start": self.top_k_start,
            "top_k_ratio_end": self.top_k_end,
            "group_fracs": np.array(GROUP_FRACS),
