...
class FedZipStrategy(fl.server.strategy.FedAvg):
    def __init__(self, test_loader, use_compression=USE_COMPRESSION, **kwargs):
        ...
        # total element count, used only to derive an "effective bits per
        # element" figure below -- FedZip has no real per-round bit-width
        # the way FGMP does (top_z is fixed, not scheduled), so this is a
        # DERIVED number (bytes actually sent -> bits/element), not a true
        # quantizer width. Kept under the same "num_bits" key purely so
        # the FGMP plotting script's bit-width panel has something to
        # plot -- read it as "effective bits/element", not "quantizer bits".
        self._total_elements = sum(v.numel() for v in self.global_model.state_dict().values())

        # Per-round history -- populated in aggregate_fit(), consumed by
        # save_run_data(). Same core keys as the FGMP/fp16/int8/fp32
        # scripts' history dicts, plus FedZip's own comm_bitpacked_bytes
        # (the sub-byte entropy-coding floor, unique to this script).
        self.history = {
            "round": [],
            "accuracy": [],
            "num_bits": [],
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        # effective bits/element this round (see comment in __init__)
        effective_bits = (round_comm_dense_bytes * 8 / self._total_elements) if self._total_elements else 0.0

        # Record this round in our own history dict (see __init__).
        self.history["round"].append(server_round)
        self.history["accuracy"].append(acc)
        self.history["num_bits"].append(effective_bits)
        ...
        self.history["comm_bitpacked_bytes"].append(round_comm_bitpacked_bytes)
    ...
    def save_run_data(self, path="fl_run_history_fedzip.npz"):
        h = self.history
        ...
        save_kwargs.update({
            ...
            "total_comm_bitpacked_bytes": self.total_comm_bitpacked_bytes,
            ...
            "top_z_weight": FEDZIP_TOP_Z_WEIGHT,
            "top_z_bias": FEDZIP_TOP_Z_BIAS,
            "n_clusters": FEDZIP_N_CLUSTERS,
        })
